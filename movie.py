...
df.select("resource", "ingesttime", "movie", "status", col("ingestdate").alias("p_ingestdate")).write.format("delta").mode("append").partitionBy("p_ingestdate").save(bronze_path)
...
df.write.mode("overwrite").option("path", "bronze_path").saveAsTable("bronze")

# COMMAND ----------

#bronze_silver
b_s = spark.read.load(bronze_path).filter("status = 'new'")
